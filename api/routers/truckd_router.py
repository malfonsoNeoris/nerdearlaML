...
truck_detector = YoloDetector(names_file,weights_file,cfg_file)
...
async def detect(data, conf=.6, nms_tresh = .5):
    ...
    classes, scores, boxes = truck_detector.DetectOnImageBase(image_readable, conf=conf, nms_tresh=nms_tresh)   
    if boxes is None:
        classes, scores, boxes = [],[],[]
    return truck_detector._encode_info(classes, scores, boxes)
